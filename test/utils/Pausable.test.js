  ...
  describe('when unpaused', function () {
    ...
    describe('when pausedFor', function () {
      ...
      it('emits a PausedFor event', async function () {
        const [unpauseDeadline] = await this.mock.getPausedForDeadlineAndTimestamp();
        await expect(this.tx).to.emit(this.mock, 'PausedFor').withArgs(this.pauser, pauseDuration, unpauseDeadline);
      });
      ...
      it('reverts when re-pausing with pauseFor', async function () {
        ...
        await expect(this.mock.pauseFor(pauseDuration - pauseDuration)).to.be.revertedWithCustomError(
          this.mock,
          'EnforcedPause',
        );
      });
      ...
      describe('unpaused', function () {
        ...
        describe('after pause duration passed', function () {
          ...
          it('reverts as contract automatically upauses', async function () {
            await expect(this.mock.unpause()).to.be.revertedWithCustomError(this.mock, 'ExpectedPause');
          });
        });
      });
      ...
      describe('pausedFor after pause duration passed', function () {
        beforeEach(async function () {
          await network.provider.send('evm_increaseTime', [pauseDuration]);
          await network.provider.send('evm_mine');
          this.tx = await this.mock.pauseFor(pauseDuration);
